...
from util import Util, FirmTypeModel
...
class BankingModel(Model):
    ...
    def __init__(self, simulation_type='HighSpread', exogenous_factors=None, number_of_banks=None):
        ...
        self.firmSizeDistribution = Util.get_firm_size(ExogenousFactors.alphaParetoDataGeneration,
                                                  ExogenousFactors.numberBanks * (ExogenousFactors.numberCorporateClientsPerBank \
                                                                                  + ExogenousFactors.numberHighRiskCorporateClientsPerBank))

        # Economy data
        self.numberBanks = ExogenousFactors.numberBanks
        self.numberFirms = 0
        self.numberHighRiskFirms = 0
        ...
        # Risk Model
        self.firmTypeModel = FirmTypeModel()
        self.y, self.X = self.firmTypeModel.generate_data()
        self.firmTypeModel.train_model(self.X, self.y)
        ...
        if ExogenousFactors.standardCorporateClients:
            _params_corporate_clients = [ExogenousFactors.minFirmSize,
                                         'LowRisk',
                                         'LowRisk',
                                         ExogenousFactors.standardCorporateClientDefaultRate,
                                         ExogenousFactors.standardCorporateClientLossGivenDefault,
                                         ExogenousFactors.standardCorporateClientLoanInterestRate]
            _params_high_risk_corporate_clients = [ExogenousFactors.minFirmSize,
                                                   'HighRisk',
                                                   'HighRisk',
                                                   ExogenousFactors.highRiskCorporateClientDefaultRate,
        ...
        firm_id = 0
        high_risk_id = 0
        total_firm_id = 0 # This is to make sure that between simulations, the firms have the same size distribution
        for bank in self.schedule.banks:
            ...
            for i in range(ExogenousFactors.numberCorporateClientsPerBank):
                if ExogenousFactors.heterogeneousRiskDistribution:
                    _params_corporate_clients[0] = self.firmSizeDistribution[total_firm_id]
                  
                    _params_corporate_clients[1] = self.firmTypeModel.generate_firm_risk_type(_params_corporate_clients[0]) 
                    
                    _params_corporate_clients[2] = self.firmTypeModel.predict_risk_type(_params_corporate_clients[0]) 
                    
                    _params_corporate_clients[3] = Util().get_random_default_probability(
                        _params_corporate_clients[0], _params_corporate_clients[1])
                else:
                    _params_corporate_clients[3] = ExogenousFactors.standardCorporateClientDefaultRate
                    
                corporate_client = CorporateClient(*_params_corporate_clients, bank, self, firm_id)
            ...
            for i in range(ExogenousFactors.numberHighRiskCorporateClientsPerBank):
                ...
                if ExogenousFactors.heterogeneousRiskDistribution:
                   _params_high_risk_corporate_clients[0] = self.firmSizeDistribution[total_firm_id]
                  
                   _params_high_risk_corporate_clients[1] = self.firmTypeModel.generate_firm_risk_type(_params_high_risk_corporate_clients[0])
                   
                   _params_high_risk_corporate_clients[2] = self.firmTypeModel.predict_risk_type(_params_high_risk_corporate_clients[0])
                   
                   _params_high_risk_corporate_clients[3] = Util().get_random_default_probability(
                        _params_high_risk_corporate_clients[0], _params_high_risk_corporate_clients[1])
                else:
                   _params_high_risk_corporate_clients[3] = ExogenousFactors.highRiskCorporateClientDefaultRate
                   
                corporate_client = CorporateClient(*_params_high_risk_corporate_clients, bank, self,
                                                   high_risk_id)
                self.schedule.add_corporate_client(corporate_client)
                high_risk_id += 1
                total_firm_id += 1
        ...
        self.renumerate_firm_ids()
 
        # Real sector Clearing House
        self.numberFirms, self.numberHighRiskFirms = self.calculate_number_of_firms()
    ...
    def calculate_number_of_firms(self):
        number_firms = 0
        number_high_risk_firms = 0
        for agent in self.schedule.corporate_clients:
            if ExogenousFactors.knowsFirmType == False:
               if agent.predictedRiskType == 'HighRisk':
                  number_high_risk_firms += 1
               else:
                  number_firms += 1
            else:
               if agent.riskType == 'HighRisk':
                  number_high_risk_firms += 1
               else:
                  number_firms += 1 
        return number_firms, number_high_risk_firms
      
    def renumerate_firm_ids(self):
        firm_id = 0
        high_risk_id = 0
...
class MyModel(BankingModel):
    ...
    def __init__(
        ...
        **kwargs):
        ...
        if firm_seed is not None:
            ExogenousFactors.firmSizeSeed = firm_seed
        if model_seed is not None:
            ExogenousFactors.modelSeed = model_seed

        ExogenousFactors.isCapitalRequirementActive = capital_requirement
        ...
        self.datacollector = DataCollector(
            ...
            agent_reporters={"Beta": lambda a: a.currentlyChosenStrategy.get_beta_value() if a.is_bank else 0,
                             ...
                             "CAR": lambda a: a.get_capital_adequacy_ratio() if a.is_bank else 0,
                             "Loan Total": lambda a: a.loanTotal if a.is_firm else 0,
                             "Firm Size": lambda a: a.firmSize if a.is_firm else 0,
                             "Risk Type": lambda a: a.riskType if a.is_firm else 0,
                             "Predicted Risk Type": lambda a: a.predictedRiskType if a.is_firm else 0,
                             'is_bank': 'is_bank',
                             'is_firm': 'is_firm'}
        )
...
def hhi(model):
    x = [i.realSectorHelper.alreadyBorrowed for i in model.schedule.corporate_clients]
    total =  np.sum([i.balanceSheet.totalNonFinancialLoans for i in model.schedule.banks])
    hhi = np.sum([(amount / total) ** 2 for amount in x])
    return hhi
...
def cr5(model):
    ...
    cr5 = np.sum(sorted_x[:5]) / total
    
    return cr5

def convergence_percentage(model):
    converged_banks = sum(1 for bank in model.schedule.banks 
                         if bank.isIntelligent and bank.check_convergence())
    total_intelligent_banks = sum(1 for bank in model.schedule.banks 
                                 if bank.isIntelligent)
    
    convergence_percentage = (converged_banks / total_intelligent_banks * 100 
                             if total_intelligent_banks > 0 else 0)
...
def simulate_monetary_experiment(base_rate, expansive_rate, n, iterations):
    if not os.path.isdir(r'simulated_results'):
        path = os.path.join(os.getcwd(), r'simulated_results')
        os.mkdir(path)
    for i in range(iterations):
        for policy in ['Base','Expansive']:
            interest = expansive_rate if policy == "Expansive" else base_rate
            ...
            ExogenousFactors.betaParamAlphaLowRisk = 50 if policy == "Expansive" else 40
            ExogenousFactors.betaParamAlphaHighRisk = 80 if policy == "Expansive" else 60
            #ExogenousFactors.betaParamAlphaLowRisk = 50
            #ExogenousFactors.betaParamAlphaHighRisk = 80
            start = time.perf_counter()
...
if __name__ == "__main__":
    if selection:
        ...
        for i in range(14, 20):
            # Distribution Seed
            ExogenousFactors.firmSizeSeed += i * 4
            ExogenousFactors.modelSeed += i
            
            # Simulation 1 - No Prudential Policy
            ...
            Modelo_original.to_csv(r'baseline/output_model_{}'.format(i) + '.csv', sep=';', decimal='.')
            data_agents.to_csv(r'baseline/output_agents_{}'.format(i) + '.csv', sep=';', decimal='.')
            
            print(r'baseline/output_model_{}'.format(i) + ' rodou!')
            ...
            Modelo_original.to_csv(r'prudential_model/output_model_1_{}'.format(i) + '.csv', sep=';', decimal='.')
            data_agents.to_csv(r'prudential_model/output_agents_1_{}'.format(i) + '.csv', sep=';', decimal='.')

            print(r'prudential_model/output_model_1_{}'.format(i) + ' rodou!')
