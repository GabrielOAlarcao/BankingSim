...
class CorporateClient(Agent):
    ...
    def __init__(self, firm_size, risk_type, predicted_risk_type,
                 default_rate,loss_given_default, loan_interest_rate, bank, model, firm_id):
        ...
        self.predictedRiskType = predicted_risk_type
        self.firmSize = firm_size
        self.firmDescountFactor = (ExogenousFactors.alphaParamCreditDemand \
                                   / (self.firmSize ** (1/ExogenousFactors.betaParamCreditDemand)))
    ...
    def credit_demand(self, interest_rate):
        loan = self.loanDemandSize * (1 - self.firmDescountFactor) - (interest_rate * self.creditElasticity)  
        return loan
