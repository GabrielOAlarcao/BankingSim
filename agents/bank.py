...
class Bank(Agent):
    ...
    def check_convergence(self, tolerance=1e-4):
        """
        Check if a bank's strategy has converged.
        Returns True if max probability is close to 1 (within tolerance).
        """
        max_prob = np.max(self.P)
        return max_prob >= (1 - tolerance)
    ...
    def get_firm_type(self, firm_size):
        return self.firmTypeModel.predict_risk_type(firm_size)

    def get_default_probability(self, firm_type, firm_size):
        if firm_type == 'HighRisk':
           default_probability = ExogenousFactors.highRiskCorporateClientDefaultRate \
                                 + Util.get_power_law(ExogenousFactors.alphaParamBank,
                                                      firm_size,
                                                      ExogenousFactors.betaParamBank)
        else:
           default_probability = ExogenousFactors.standardCorporateClientDefaultRate \
                                 + Util.get_power_law(ExogenousFactors.alphaParamBank,
                                                      firm_size,
                                                      ExogenousFactors.betaParamBank)
    ...
    def get_real_sector_interest_rate(self, firm_type, firm_size):
        strategy = self.currentlyChosenStrategy
        mu = strategy.get_MuR_value()
        exo_rate = ExogenousFactors.liquidAssetsInterestRate
        p = self.get_default_probability(firm_type, firm_size)
        c = self.get_wacc()
