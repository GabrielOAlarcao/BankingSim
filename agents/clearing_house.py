...
class RealSectorClearingHouse(Agent):
    ...
    def __init__(self, number_banks, number_firms, number_high_risk_firms,
                 firm_type_model, model):
        super().__init__(Util.get_unique_id(), model)
        ...
        self.realSectorFirmSizeMatrix = np.zeros((self.numberBanks, self.numberFirms))
        ...
        self.highRiskFirmSizeMatrix = np.zeros((self.numberBanks, self.numberHighRiskFirms))
        ...
        if len(self.banksOfferingCredit) > 0 and len(self.firmsDemandingCredit) > 0:
            ...
            while True:
                try:
                    ...
                    bank_variables = lender.get_real_sector_interest_rate('LowRisk', 
                                                                          borrower.firmSize)
                    interest_rate = bank_variables[0]
                    ...
                    self.realSectorFirmSizeMatrix[lender_id, borrower_id] = borrower.firmSize
                    self.realSectorInterestMatrix[lender_id, borrower_id] = interest_rate
                    self.defaultProbabilityMatrix[0, borrower_id] = bank_variables[1]
        ...
        if len(self.banksOfferingHighRiskCredit) > 0 and len(self.highRiskFirmsDemandingCredit) > 0:
            ...
            while True:
                try:
                    ...
                    bank_variables = lender.get_real_sector_interest_rate('HighRisk',
                                                                          borrower.firmSize)
                    interest_rate = bank_variables[0]
                    ...
                    self.highRiskFirmSizeMatrix[lender_id, borrower_id] = borrower.firmSize
                    self.highRiskInterestMatrix[lender_id, borrower_id] = interest_rate
                    self.highRiskDefaultProbabilityMatrix[0, borrower_id] = bank_variables[1]
                    self.defaultLGDMatrix[0, borrower_id] = borrower.lossGivenDefault
